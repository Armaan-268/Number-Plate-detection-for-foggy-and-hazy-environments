...
save = './Hazed'
save2 = './Hazed2'
for img in os.listdir(do):
    path = os.path.join(do,img)

    natural_img = cv2.imread(path)
    ...
    cv2.imwrite(os.path.join(save,img), final_img)
    cv2.imwrite(os.path.join(save2,img), final_img2)
# Display the result
